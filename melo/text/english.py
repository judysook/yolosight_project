...
def g2p(text, pad_start_end=True, tokenized=None):
    # 간단히 알파벳 단위로 분해
    phones = list(text.lower().replace(" ", ""))
    tones = [0] * len(phones)
    
    # tokenizer는 여전히 사용 가능 (bert tokenizer)
    if tokenized is None:
        tokenized = tokenizer.tokenize(text)
    
    word2ph = [1 for _ in tokenized]

    # 보정: phones 길이와 word2ph 길이 불일치 방지
...
def get_bert_feature(text, word2ph, device=None):
    return None  # nltk 없이 사용하기 위해 BERT 기능 생략
